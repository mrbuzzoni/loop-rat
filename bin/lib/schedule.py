...
def cmd_cron(path, root):
    """One crontab line, not one per loop.
    ...
    print("# loop rat - generated by `bin/rat cron`, safe to paste into crontab")
    print("# the schedule itself lives in .claude/loops/schedule.yml:")
    for loop in loops:
        print("#   %-16s every %-5s %-14s %-8s %s" % (
            loop["name"], loop.get("every", "-"),
            loop.get("window", "any hour"), loop.get("days", "all days"),
            "" if loop.get("enabled", True) else "(paused)"))
    print("# edit that file and the next tick picks it up - no crontab change needed.")
    if tick >= 60:
        expr = "0 * * * *"
    else:
        expr = "*/%d * * * *" % tick
...
def main(argv):
    ...
    if command == "cron":
        return cmd_cron(path, argv[3] if len(argv) > 3 else os.getcwd())
